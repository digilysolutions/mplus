                                ...
                                <tbody>
                                 ...
                                 @endphp
                                    ...
                                    @foreach ($municipalities as $municipality)
										...
										<td >{{ $municipality->province_id }}</td>
										<td >{{ $municipality->is_activated }}</td>

                                            <td>
                                                <form action="{{ route('municipalities.destroy', $municipality->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('municipalities.show', $municipality->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Mostrar') }}</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('municipalities.edit', $municipality->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Editar') }}</a>
                                                    @csrf
                                                    @method('DELETE')
