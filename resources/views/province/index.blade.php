                                ...
                                <tbody>
                                 ...
                                 @endphp
                                    ...
                                    @foreach ($provinces as $province)
										...
										<td >{{ $province->is_activated }}</td>

                                            <td>
                                                <form action="{{ route('provinces.destroy', $province->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('provinces.show', $province->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Mostrar') }}</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('provinces.edit', $province->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Editar') }}</a>
                                                    @csrf
                                                    @method('DELETE')
