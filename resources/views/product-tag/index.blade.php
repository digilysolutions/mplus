                                ...
                                <tbody>
                                 ...
                                 @endphp
                                    ...
                                    @foreach ($productTags as $productTag)
										...
										<td >{{ $productTag->is_activated }}</td>

                                            <td>
                                                <form action="{{ route('product-tags.destroy', $productTag->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('product-tags.show', $productTag->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Mostrar') }}</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('product-tags.edit', $productTag->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Editar') }}</a>
                                                    @csrf
                                                    @method('DELETE')
